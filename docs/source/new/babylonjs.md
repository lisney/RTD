...
.substr(4,9) //빼내기, 4번째 인덱스부터 9번째까지 반환(인수 9가 없으면 4이후 전부 반환), 음수사용 불가
.slice(-4), (-4,-2) - -4
.split('-') //문자 '-'를 기준으로 분리한 요소를 배열로 반환(<=> 배열.join('--') 문자열을 배열로(.split(''))
.splice(0,1) : 배열에서 0~1번째 인덱스 요소를 삭제하고 삭제한 요소를 리턴(배열에만 사용가능)=> 객체.slice 배열(문자열) 원본은 보존/리턴

.trim() 앞 뒤 공백문자 제거 '   0ㅇㄹㅇ  ' -> '0ㅇㄹㅇ'
.padStart(자릿수,채울문자(열) : (5,'0') -> '00056'
-----------------------------------------------------------------
